...
#  def self.run argv = ARGV
  ...
  # generate key/IV
  if gen_key_path
    KeyLoader.create_key_iv_file(gen_key_path)
  ...
  if password_file
#    enc_password = Base64.decode64(IO.readlines(password_file)[0].chomp)
#    decrypted = PasswordService.decrypt(enc_password, key_path)
        decrypted = PasswordService.getPassword(password_file, key_path)
  ...
  if output_path
    ...
    PasswordService.write_password_file(password, output_path, key_path)
